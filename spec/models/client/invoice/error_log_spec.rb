...
RSpec.describe Client::Invoice::ErrorLog, type: :model do
  describe 'scopes' do
    let!(:error_log_this_year) { create(:client_invoice_error_log, date: Date.today) }
    let!(:error_log_last_year) { create(:client_invoice_error_log, date: 1.year.ago) }
    let!(:error_log_this_month) { create(:client_invoice_error_log, date: Date.today) }
    let!(:error_log_last_month) { create(:client_invoice_error_log, date: 1.month.ago) }

    it 'filters records for the current year' do
      logs_this_year = Client::Invoice::ErrorLog.range_year(Date.today)
    ...
    it 'filters records for the current month' do
      logs_this_month = Client::Invoice::ErrorLog.range_month(Date.today)
